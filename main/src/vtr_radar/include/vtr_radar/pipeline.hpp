...
/**
 * \file pipeline.hpp
 * \brief RadarPipeline class definition
 *
...
class RadarPipeline : public tactic::BasePipeline {
 public:
  ...
  RadarPipeline(const std::string &name = static_name)
 ...
 private:
  ...
  void setOdometryPrior(RadarQueryCache::Ptr &qdata,
                        const tactic::Graph::Ptr &graph);

  void savePointcloudMap(RadarQueryCache::Ptr qdata,
                         const tactic::Graph::Ptr graph,
 ...
 private:
   ...
   */
  RadarQueryCache::Ptr candidate_qdata_ = nullptr;
  ...
  /** \brief Current map and its vertex for localization */
  ...
  /**
   * \brief a trajectory to estimate transform at a future time
