...
/**
 * \file pipeline.cpp
 * \brief RadarPipeline class methods definition
 *
...
void RadarPipeline::configFromROS(const rclcpp::Node::SharedPtr &node,
...
void RadarPipeline::initialize(const Graph::Ptr &) {
  // preprocessing
...
void RadarPipeline::preprocess(QueryCache::Ptr &qdata0,
                               const Graph::Ptr &graph) {
  for (auto module : preprocessing_) module->run(*qdata0, graph);
}

void RadarPipeline::visualizePreprocess(QueryCache::Ptr &qdata0,
                                        const Graph::Ptr &graph) {
  for (auto module : preprocessing_) module->visualize(*qdata0, graph);
}

void RadarPipeline::runOdometry(QueryCache::Ptr &qdata0,
                                const Graph::Ptr &graph) {
...
void RadarPipeline::visualizeOdometry(QueryCache::Ptr &qdata0,
                                      const Graph::Ptr &graph) {
  for (auto module : odometry_) module->visualize(*qdata0, graph);
}

void RadarPipeline::runLocalization(QueryCache::Ptr &qdata0,
                                    const Graph::Ptr &graph) {
  auto qdata = std::dynamic_pointer_cast<RadarQueryCache>(qdata0);
...
void RadarPipeline::visualizeLocalization(QueryCache::Ptr &qdata0,
                                          const Graph::Ptr &graph) {
  for (auto module : localization_) module->visualize(*qdata0, graph);
}

void RadarPipeline::processKeyframe(QueryCache::Ptr &qdata0,
...
/// issues. Also localizing against a map is a read only operation.
  ...
  CLOG(DEBUG, "radar.pipeline")
      << "[Radar Pipeline] Launching the point cloud map saving thread.";
  ...
  map_saving_thread_future_ =
      std::async(std::launch::async, [this, qdata, graph, live_id]() {
        el::Helpers::setThreadName("radar.pipeline.map_saving");
        savePointcloudMap(qdata, graph, live_id);
      });
#endif
}

void RadarPipeline::wait() {
  std::lock_guard<std::mutex> lck(map_saving_mutex_);
...
void RadarPipeline::setOdometryPrior(RadarQueryCache::Ptr &qdata,
                                     const Graph::Ptr &graph) {
...
void RadarPipeline::savePointcloudMap(RadarQueryCache::Ptr qdata,
                                      const Graph::Ptr graph,
                                      VertexId live_id) {
  CLOG(DEBUG, "radar.pipeline")
      << "[Radar Pipeline] Start running the point map saving thread.";
  ...
  CLOG(DEBUG, "radar.pipeline")
      << "[Radar Pipeline] Finish running the point map saving thread.";
}
